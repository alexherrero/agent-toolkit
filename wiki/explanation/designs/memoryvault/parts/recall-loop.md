...
1. **Embed the query** — local `sentence-transformers` (BGE-large default; ~1.3GB checkpoint cached at `~/.cache/agent-toolkit/sentence-transformers/`; PyTorch MPS on Apple Silicon for acceleration) as of v0.9.2 (see [ADR 0001's 2026-05-20 amendment](../../decisions/0001-agent-toolkit-purpose.md#amendment-2026-05-20)). Synchronous call; on time-budget overrun, fall back to grep-only.
...
6. **Offline-capable recall works** — verify recall succeeds with network disconnected once BGE-large model is cached locally (v0.9.2 is local-only by default — see [ADR 0001 amendment](../../decisions/0001-agent-toolkit-purpose.md#amendment-2026-05-20); no API path exists to fall back to).
